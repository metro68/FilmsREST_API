...
public class FilmControllerTest {
        ...
        public void allTest() throws Exception {
                ...
                mvc.perform(get("/api/films/allFilms")
                                ...
                                // Uses ._embedded.filmList due to the assembler used in this mapping
                                .andExpect(jsonPath("$._embedded.filmList[*].id").isNotEmpty())// [*] checks all ids.
                                                                                               // Entire line
                                                                                               // ensures all ids are
                                // present as expected
                                ...
                                .andExpect(jsonPath("$._embedded.filmList[?(@.title == 'Andres De Fonollosa')]")
                                                .doesNotExist())
                                .andExpect(jsonPath("$._embedded.filmList[?(@.subject == 'Crimewar')]").doesNotExist())
                                .andExpect(jsonPath("$._embedded.filmList[?(@.actor == 'Berlin Alonso')]")
                                                .doesNotExist())
                                .andExpect(jsonPath("$._embedded.filmList[?(@.actress == 'Tokyo Corbero')]")
                                                .doesNotExist())
                                .andExpect(jsonPath("$._embedded.filmList[?(@.director == 'Pina Denver')]")
                                                .doesNotExist())
        ...
        public void deleteFilmTest() throws Exception {
                ...
                String filmsBeforeInJson = mvc.perform(get("/api/films/allFilms"))
                                .andReturn().getResponse().getContentAsString();

                // ObjectMapper objectMapperBefore = new ObjectMapper();

                // Convert JSON response in string to a list to count the numbers before and
                // after deletion
                List<String> responseBefore = new ArrayList<String>(Arrays.asList(filmsBeforeInJson.split("\\{")));
                // Films responseBefore = objectMapperBefore.readValue(filmsBeforeInJson,
                // Films.class);

                final int noOfFilmsBefore = responseBefore.size();

                mvc.perform(delete("/api/films/{id}", 100))
                                .andExpect(status().isOk());

                String filmsAfterInJson = mvc.perform(get("/api/films/allFilms"))
                                .andReturn().getResponse().getContentAsString();

                // ObjectMapper objectMapperAfter = new ObjectMapper();

                // Convert to Java object in Jackson
                // Films responseAfter = objectMapperAfter.readValue(filmsAfterInJson,
                // Films.class);

                // `\\{` used because `{` is for repetition sequences in regex
                List<String> responseAfter = new ArrayList<String>(Arrays.asList(filmsAfterInJson.split("\\{")));
                final int noOfFilmsAfter = responseAfter.size();

                // System.out.println("string" + responseBefore.size());
                // System.out.println("string" + responseAfter.size());
                // System.out.println("string" + responseBefore);
                // System.out.println("string" + responseAfter);

                // we subtract 4 because of links present in the JSON response, they make each
                // film return with: the film, its self link and to href links, i.e 4 items each
                assertEquals(noOfFilmsBefore - 4, noOfFilmsAfter);
        }
}